...
let updateEmployeeManager = () => {
    db.query("SELECT * FROM employee;", (err, data) => {
        updateEmpManagerQuestion[0].choices = data.map((element) => ({ value: element.id, name: element.first_name + " " + element.last_name }));
        db.query("SELECT * FROM employee;", (err, data) => {
            updateEmpManagerQuestion[1].choices = data.map((element) => ({ value: element.id, name: element.first_name + " " + element.last_name }));
            updateEmpManagerQuestion[1].choices.push({ value: null, name: "None" });
            inquirer.prompt(updateEmpManagerQuestion)
                .then((response) => {
...
let viewByDepartment = () => {
    db.query("SELECT * FROM department;", (err, data) => {
        ...
        inquirer.prompt(viewByDepartmentQuestions)
            .then((response) => {
                db.query("SELECT first_name, last_name, name AS department FROM employee JOIN roles ON roles.id = employee.roles_id JOIN department ON department.id = roles.department_id WHERE department.id = ?;", [response.whichDepartment], (err, data) => {
                    console.table(data);
                    askInit();
                })
            })
